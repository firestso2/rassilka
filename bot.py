...
class AddAccountByPhone(StatesGroup):
    # Режим "по коду" — только номер, API берётся из .env
    waiting_phone = State()
    waiting_code = State()
    waiting_2fa = State()
...
async def accounts_list(msg: Message):
    ...
    add_btns = [
        [InlineKeyboardButton(text="📲 Добавить по номеру телефона", callback_data="acc_add_phone")],
        [InlineKeyboardButton(text="🔑 Добавить по API ID + Hash", callback_data="acc_add_api")],
    ]
    if not accounts:
        await msg.answer(
            "У вас нет добавленных аккаунтов.\nВыберите способ добавления:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=add_btns)
        )
    else:
        kb = kb_accounts(accounts)
        kb.inline_keyboard.extend(add_btns)
        await msg.answer("Ваши аккаунты:", reply_markup=kb)


@dp.callback_query(F.data == "acc_add_api")
...
@dp.callback_query(F.data == "acc_add_phone")
async def cb_acc_add_phone(cb: CallbackQuery, state: FSMContext):
    from config import SHARED_API_ID, SHARED_API_HASH
    if not SHARED_API_ID or not SHARED_API_HASH:
        await cb.message.answer(
            "❌ В конфиге не заданы <code>SHARED_API_ID</code> и <code>SHARED_API_HASH</code>.\n"
            "Добавьте их в <code>.env</code> файл и перезапустите бота.",
            parse_mode="HTML"
        )
        await cb.answer()
        return
    await cb.message.answer(
        "📲 Введите номер телефона аккаунта (формат: <code>+79001234567</code>):",
        parse_mode="HTML"
    )
    await state.set_state(AddAccountByPhone.waiting_phone)
    await cb.answer()


# ── По номеру (SHARED API) ─────────────────────────────────────────────────

@dp.message(AddAccountByPhone.waiting_phone)
async def phone_auth_got_phone(msg: Message, state: FSMContext):
    from config import SHARED_API_ID, SHARED_API_HASH
    phone = msg.text.strip()
    status_msg = await msg.answer("⏳ Отправляю код на Telegram...")
    try:
        phone_hash = await ubm.send_code(SHARED_API_ID, SHARED_API_HASH, phone)
        await state.update_data(phone=phone, phone_hash=phone_hash,
                                api_id=SHARED_API_ID, api_hash=SHARED_API_HASH)
        await status_msg.edit_text(
            "📩 Код отправлен в Telegram.\n\n"
            "Введите код в формате <code>12345</code> (без пробелов):",
            parse_mode="HTML"
        )
        await state.set_state(AddAccountByPhone.waiting_code)
    except Exception as e:
        await status_msg.edit_text(f"❌ Ошибка при отправке кода: <code>{e}</code>", parse_mode="HTML")
        await state.clear()


@dp.message(AddAccountByPhone.waiting_code)
async def phone_auth_got_code(msg: Message, state: FSMContext):
    ...
    try:
        ...
        if result == "2fa":
            await status_msg.edit_text("🔐 Требуется пароль двухфакторной аутентификации.\nВведите пароль:")
            await state.set_state(AddAccountByPhone.waiting_2fa)
        else:
            ...
            await status_msg.edit_text(
                f"✅ Аккаунт <code>{data['phone']}</code> успешно добавлен и подключён!",
                parse_mode="HTML"
            )
            await state.clear()
    except Exception as e:
...
@dp.message(AddAccountByPhone.waiting_2fa)
async def phone_auth_got_2fa(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await status_msg.edit_text(f"❌ Неверный пароль: <code>{e}</code>", parse_mode="HTML")
        await state.clear()
